...
braces (C<{> and C<}>). Matching pairs of curly braces can appear unquoted
in the string (even nested), but they can be quoted if necessary with a
preceding backslash character (C<"\">). Backslashes can also be quoted with
a preceding backslash. Any backslashes that do not precede a curly brace or
another backslash are treated as normal characters, and are included in the
string.
