...
I<C>'s conditional, logical, relational, equality, arithmetic, and bitwise
...
With the I<sequence> operator (C<,>), both operands are executed. The value
of the first operand is discarded. The result is the value of the second
operand.
...
evaluated. It is also important when using the C<texists> built-in, the
C<sh> and C<ush> "pattern" modifiers, and the C<exists> reference file field
(see below).
...
(i.e.,
S<C<[>I<yyyy>C</>I<mm>C</>I<dd>C<]>> and
S<C<[>I<yyyy>C</>I<mm>C</>I<dd> I<hh>C<:>I<mm>C<:>I<ss>C<.>I<nnnnnnnnn>C<]>>). They
are interpreted as being in the local time zone. If the year is specified as
a number below 100, then it is interpreted as being a year in the current
century. Hours must be in 24-hour time, not 12-hour time. Trailing zero
digits of the nanoseconds component are optional. The nanoseconds component
itself is optional if it is zero (i.e.,
S<C<[>I<yyyy>C</>I<mm>C</>I<dd> I<hh>C<:>I<mm>C<:>I<ss>C<]>>). The seconds
component is optional if it is also zero (i.e.,
S<C<[>I<yyyy>C</>I<mm>C</>I<dd> I<hh>C<:>I<mm>C<]>>). The minutes component
is optional if it is also zero (i.e.,
S<C<[>I<yyyy>C</>I<mm>C</>I<dd> I<hh>C<]>>). The hours component is optional
if it is also zero (i.e., midnight). Then, it's just a date. Dates and
date/times evaluate to the number of nanoseconds since the I<UNIX> Epoch.
...
for details. It is an error if the nanosecond value is outside its valid
range.
...
C<st_atime> field of the corresponding I<stat(2)> structure converted to
nanoseconds, and combined with the nanoseconds from C<st_atim.tv_nsec> (or
equivalent).

=item C<mtime>

The time that the current candidate file's contents were last modified. This
is the C<st_mtime> field of the corresponding I<stat(2)> structure converted
to nanoseconds, and combined with the nanoseconds from C<st_mtim.tv_nsec>
(or equivalent).

=item C<ctime>

The time that the current candidate file's inode was last changed. This is
the C<st_ctime> field of the corresponding I<stat(2)> structure converted to
nanoseconds, and combined with nanoseconds from C<st_ctim.tv_nsec> (or
equivalent).

=item C<btime>

The time that the current candidate file was created. This is known as the
file's birth or created time. This is the C<st_birthtim> timestamp field of
the corresponding I<stat(2)> structure (or equivalent) converted to
nanoseconds.

This is not supported on all operating systems (i.e., I<OpenBSD> and
I<NetBSD> lack support), or by all filesystems. When birth times are not
supported, this is zero, or in other words, the I<UNIX> Epoch.
...
This is also available on I<FreeBSD>, I<OpenBSD>, I<NetBSD>, I<macOS>, and
I<Solaris>. The attributes/flags that are available vary from system to
system. See I<chflags(1)> (or I<chmod(1)> on I<Solaris>) for details. See
C</etc/rawhide.conf.d/attributes> for all the related predicate functions
(e.g., C<uimmutable>, C<uappend>, C<nodump>, C<opaque>, ...).

On other systems (e.g., I<Cygwin>), this doesn't exist at all.
...
The current time (when I<rh(1)> started) in nanoseconds since the I<UNIX>
Epoch.

Note that timers on different systems have different resolutions. I<Linux>,
I<FreeBSD>, I<OpenBSD>, and I<NetBSD> have nanosecond precision. I<macOS>
and I<Cygwin> have microsecond precision. I<Solaris> has millisecond
precision.

=item C<today>

The time last midnight (local time) in nanoseconds since the I<UNIX> Epoch.
...
The average number of nanoseconds in a month.

=item C<year>

The average number of nanoseconds in a year.
...
=item C<IFREG>, C<IFDIR>, C<IFLNK>, C<IFCHR>, C<IFBLK>, C<IFSOCK>, C<IFIFO>, C<IFDOOR>, C<IFWHT>
...
device, socket, fifo/pipe, door, and whiteout, respectively). C<IFDOOR> only
applies to I<Solaris>. C<IFWHT> only applies to I<macOS>.
...
structure converted to nanoseconds, and combined with the nanoseconds from
C<st_atim.tv_nsec> (or equivalent).
...
I<stat(2)> structure converted to nanoseconds, and combined with the
nanoseconds from C<st_mtim.tv_nsec> (or equivalent).
...
structure converted to nanoseconds, and combined with the nanoseconds from
C<st_ctim.tv_nsec> (or equivalent).
...
The time that the current candidate symlink's ultimate target was
created. This is known as the target's birth or created time. This is the
C<st_birthtim> timestamp field of the corresponding I<stat(2)> structure (or
equivalent) converted to nanoseconds.
...
C<.profile>. This is unlike C</bin/sh> globbing behaviour, but it is like
...
would match C<.profile>. This is unlike C</bin/sh> globbing behaviour, but
it is like I<POSIX find(1)> behaviour.
...
(C<"\">). Note: When quoting a backslash in the pattern with a preceding
backslash, both backslashes need to be quoted in the string literal with
backslashes.

The following subset of I<ksh> extended glob pattern notation is supported
on platforms with S<I<GNU glibc>> (i.e., I<Linux>):
...
The special meanings of C<?>, C<*>, C<+>, C<@>, C<!>, C<(>, C<|>, C<)>, and
C<\> can be removed by quoting them with a preceding backslash character
(C<"\">). Note: When quoting a backslash in the pattern with a preceding
backslash, both backslashes need to be quoted in the string literal with
backslashes.
...
from the search directory instead. This behaviour can be suppressed by
...
use I<grep(1)> via the C<sh> or C<ush> "pattern" modifiers instead.
...
Also note that this only works well with regular files that are readable to
the current user.
...
The output of S<C<rh --help>> states if case-insensitive glob pattern
matching is available on the local system.
...
full path starting from the search directory instead. This behaviour can be
suppressed by setting the environment variable
C<RAWHIDE_NO_IMPLICIT_PATH_MODIFIER=1>.
...
C<reacl>, C<redacl>, C<reea>, C<rewhat>, and C<rebody>). These are all
considered to be multiple lines. It can be enabled by default in all cases
by setting the environment variable C<RAWHIDE_PCRE2_DOTALL_ALWAYS=1>. It can
be enabled for individual matches by starting the regular expression with
C<(?s)>. It can be disabled for individual matches by starting the regular
expression with C<(?-s)>. Note that, even when C<.> doesn't match a newline,
there are other patterns that still match a newline (e.g., C<\s>, C<\v>,
C<\H>, C<\W>, C<\D>, C<[^anything-that-does-not-include-newline]>).
...
else (e.g., C<reacl>, C<redacl>, C<reea>, C<rewhat>, and C<rebody>). These
are all considered to be multiple lines. It can be enabled by default in all
cases by setting the environment variable
C<RAWHIDE_PCRE2_MULTILINE_ALWAYS=1>. It can be enabled for individual
matches by starting the regular expression with C<(?m)>. It can be disabled
for individual matches by starting the regular expression with C<(?-m)>.
...
is made when the locale uses I<UTF-8> (i.e., when the environment variable
C<LC_ALL>, C<LC_CTYPE>, or C<LANG>, includes C<"UTF-8">). When the locale
doesn't use I<UTF-8>, and you want I<rh> to assume that everything is
...
the file's full path starting from the search directory instead. This
behaviour can be suppressed by setting the environment variable
C<RAWHIDE_NO_IMPLICIT_PATH_MODIFIER=1>.
...
But if you need to match binary data in a binary file, when the current
locale uses I<UTF-8>, you will need to set the environment variable
C<RAWHIDE_PCRE2_NOT_UTF8_DEFAULT=1> (or C<LANG=C>) so as to prevent assuming
that the file is encoded in I<UTF-8> as described above.
...
Apply the glob pattern to each candidate directory's default access control
list (ACL), rather than its base name. This is only meaningful for
directories.

This is available on systems with I<"POSIX"> ACLs (e.g., I<Linux>,
I<FreeBSD> (for I<ufs>), and I<Cygwin>), but not on systems with I<NFSv4>
ACLs (e.g., I<FreeBSD> and I<Solaris> (for I<zfs>)), or S<I<macOS Extended>>
ACLs. I<OpenBSD> and I<NetBSD> don't do ACLs. See I<acl(2)> or I<acl(3)> or
I<acl(5)> for more details.

The output of S<C<rh --help>> states if default access control lists are
available on the local system.

See C<acl> above for details on the content of I<"POSIX"> access control
lists.

Note that glob patterns are anchored to the start and end, and so must match
the entire text.

=item C<idacl> (or C<id>)

Like the C<dacl> modifier, but apply the glob pattern without
case-sensitivity. This might not be available on all systems.

=item C<redacl> (or C<red>)

Like the C<dacl> modifier, but match each candidate directory's default
access control list (ACL) against a I<Perl>-compatible regular expression
(regex) instead of a glob pattern. By default, the directory's default ACL
is considered to be multiple lines (i.e., as if the regular expression
started with C<(?m)>, or the I<Perl> C</m> modifier were used). This is
available on systems that have I<libpcre2-8> installed, and I<"POSIX">
access control lists. See the C<re> modifier above for more information.

=item C<reidacl> (or C<reid>)

Like the C<redacl> modifier, but apply the regex without case-sensitivity.

=item C<ea> (or C<e>)

Apply the glob pattern to the encoded extended attributes (EA) text of each
candidate file, rather than its base name.
...
The encoded extended attributes text, if any, will be possibly multiple
lines, one per extended attribute, each terminated by a newline
character. Each extended attribute starts with its name. If it also has a value, then
the name is followed by colon and space characters (S<C<": ">>), and then
the value. Note that on I<macOS>, some extended attribute names contain a
colon character, but they aren't typically followed by a space character.
...
backslash characters are quoted with a preceding backslash. Any extended
attribute names that contain S<C<": ">> have the colon encoded as the
hexadecimal escape sequence C<"\x3a"> so as to eliminate any risks from
malicious extended attribute names. Note that commas are not quoted.
...
I<atime>, and I<ctime> are in I<ISO> date/time format
(I<yyyy>C<->I<mm>C<->I<dd>C<T>I<hh>C<:>I<mm>C<:>I<ss+hhmm>).
...
Like the C<ea> modifier, but match the encoded extended attributes (EA) text
of each candidate file against a I<Perl>-compatible regular expression
(regex) instead of a glob pattern. By default, the file's EAs are considered
to be multiple lines (i.e., as if the regular expression started with
C<(?m)>, or I<Perl>'s C</m> modifier were used). This is available on
systems that have I<libpcre2-8> installed, and supported extended
attributes. See the C<re> modifier above for more information.
...
external shell command to execute as if by I<system(3)> (i.e., by C</bin/sh>
or some other I<POSIX>-compliant shell). It evaluates to the exit success
status of the external shell command. If its exit status is zero, then the
value is true (i.e., non-zero). Otherwise, it is false (i.e., zero).
...
Like the C<sh> modifier (see above), except that the command string is
executed by the user's login shell, as specified in the C</etc/passwd> file,
rather than as if by I<system(3)> (i.e., by C</bin/sh> or some other
I<POSIX>-compliant shell).

If the environment variable C<RAWHIDE_USER_SHELL> is set, its value must
specify the absolute path to an alternative shell to use instead of the
user's login shell. This is only available to non-C<root> users (as it could
be dangerous for C<root>). The alternative shell will be invoked in the same
way that the user's login shell would be, which is the same way that
C</bin/sh> is invoked by the C<sh> modifier.

The user's login shell (or alternative shell) must accept a C<-c> option
which causes it to interpret its first non-option argument as a command
string to execute. It must also accept additional command line arguments
that represent the shell's base name, and the positional parameters for the
command. An exception to this requirement is made for I<csh>, I<tcsh>, and
I<fish>, whose first argument after the command is the command's first
positional parameter, rather than the shell's base name. If the user's login
shell (or alternative shell) isn't named C<csh> or C<tcsh> or C<fish>, and
doesn't take an argument to represent the shell's base name, then you must
set the environment variable C<RAWHIDE_USER_SHELL_LIKE_CSH=1> to cause I<rh>
to invoke it without that argument. The user's login shell (or alternative
shell) must also have command syntax such that C<"$1"> and C<"$2">
represent, respectively, the first and second positional arguments passed to
it on its command line. An exception to this requirement is made for
I<fish>, whose corresponding syntax is C<$argv[1]> and C<$argv[2]>. Any
other non-I<POSIX> shells can only be used if you promise to never use the
C<%s> or C<%S> notation, but instead always use the shell's own notation for
positional parameters, whatever that might be. Note that such shells will
usually also require that you set the environment variable
...
(nor can the path to the shell itself). No shell syntax is supported. It is
just a vector of whitespace-separated strings.

B<Warning:> Search criteria expressions that use the C<ush> "pattern"
modifier are not portable.

B<Warning:> This can be much slower than the C<sh> modifier, depending on
the choice of shell. Based on a trivial test on I<Linux>, here are some
timing comparisons against C</bin/sh>: I<ksh> is 1.64 times slower, I<bash>
is 1.67 times slower, I<csh> is 5.48 times slower, I<tcsh> is 5.53 times
slower, I<fish> is 7.10 times slower, and I<zsh> is 21.85 times slower. But
the language features of these or other shells might make it worthwhile
anyway.

B<Warning>: Here be mighty dragons. With the use of a wrapper script, this
even allows access to other languages that don't match the standard login
shell invocation convention. For example, the following wrapper script would
allow you to use I<perl> as the user shell:

    #!/bin/sh
    # usage (via rh .ush): perlsh -c -- $cmd $0 $1 $2
    exec /usr/bin/perl -E "$3" "$5" "$6"

But this is getting silly now. Although, it's only 2.73 times slower than
C</bin/sh>, so maybe it's not so silly. The I<python3> equivalent (no
wrapper script needed, just C<RAWHIDE_USER_SHELL_LIKE_CSH=1>) is 16.72 times
slower than C</bin/sh>. But don't read too much into these timings. They
aren't meaningful tests. They're only testing process startup times.
...
field of the corresponding I<stat(2)> structure converted to nanoseconds,
and combined with the nanoseconds from C<st_atim.tv_nsec> (or equivalent).
...
C<st_mtime> field of the corresponding I<stat(2)> structure converted to
nanoseconds, and combined with the nanoseconds from C<st_mtim.tv_nsec> (or
equivalent).
...
C<st_ctime> field of the corresponding I<stat(2)> structure converted to
nanoseconds, and combined with the nanoseconds from C<st_ctim.tv_nsec> (or
equivalent).
...
The time that the reference file was created. This is known as the file's
birth or created time. This is the C<st_birthtim> timestamp field of the
corresponding I<stat(2)> structure (or equivalent) converted to nanoseconds.
...
This is the default C</etc/rawhide.conf> standard library:
  ...
  */
  ...
  namedpipe { fifo }
  pipe      { fifo }
  whiteout  { wht || (chr && !rdev) } # macOS, Linux
  ...
  created  { btime }
  birth    { btime }
  len      { strlen } # Length of the base name
  sz       { size }
  
  seconds  { second } # Nanoseconds
  minutes  { minute }
  hours    { hour }
  days     { day }
  weeks    { week }
  ...
  tnamedpipe { tfifo }
  tpipe      { tfifo }
  twhiteout  { twht || (tchr && !trdev) } # macOS, Linux
  ...
  target_pipe      { tfifo }
  target_whiteout  { twhiteout } # macOS, Linux
  ...
  target_p { tfifo }
  target_D { tdoor }
  target_W { twhiteout } # macOS, Linux
  ...
  tcreated  { tbtime }
  tbirth    { tbtime }
  tlen      { tstrlen } # Length of the target's base name
  tsz       { tsize }
...
On I<FreeBSD> systems, this is the C</etc/rawhide.conf.d/attributes>
library. There is a similar file on I<OpenBSD>, I<NetBSD>, I<macOS>, and
I<Solaris> systems:
...
I<rh(1)>, I<stat(2)>, I<statx(2)>, I<getattrat(3C)>, I<mktime(3)>,
I<fnmatch(3)>, I<glob(7)>, I<perlre(1)>, I<pcre2pattern(3)>,
I<pcre2syntax(3)>, I<access(2)>, I<acl(2)> or I<acl(3)> or I<acl(5)>,
I<xattr(1)> or I<xattr(7)> or I<extattr(2)> or I<fsattr(7)>, I<chattr(1)>,
I<lsattr(1)>, I<chflags(1)>, I<file(1)>, I<jq(1)>, S<I<The C programming
language>>.
