...
expression (regex) instead of a glob pattern. See I<perlre(1)>,
I<pcre2pattern(3)>, and I<pcre2syntax(3)> for details. This is available on
systems that have I<libpcre2-8> installed.
...
The following I<PCRE2> options are enabled by default. Unless otherwise
stated, these options are always enabled.
...
target paths, or MIME types (e.g., `re`, `repath`, `relink`, `remime`)
because it can be helpful in the face of malicious paths. It is not enabled
by default when matching everything else (e.g., `reacl`, `reea`, `rewhat`,
`rebody`). It can be enabled in all cases by setting the environment
variable C<RAWHIDE_PCRE2_DOTALL_ALWAYS=1>.
...
symlink target paths, or MIME types (e.g., `re`, `repath`, `relink`,
`remime`) because it can be unhelpful in the face of malicious paths. It is
only enabled by default when matching everything else (e.g., `reacl`,
`reea`, `rewhat`, `rebody`). It can be enabled in all cases by setting the
environment variable C<RAWHIDE_PCRE2_MULTILINE_ALWAYS=1>.
...
path starting from the search directory, not just its base name.
...
target path, rather than its base name.

=item C<rebody> (or C<reb>)

Like the C<re> modifier, but apply the regex to each candidate file's
contents, rather than its base name. See C<body> above for more details.

=item C<rewhat> (or C<rew>)

Like the C<re> modifier, but apply the regex to each candidate file's file
type description, rather than its base name. This is available on systems
that have I<libpcre2-8> and I<libmagic> installed. See C<what> above for
more details.

=item C<remime> (or C<rem>)

Like the C<re> modifier, but apply the regex to each candidate file's MIME
type, rather than its base name. This is available on systems that have
I<libpcre2-8> and I<libmagic> installed. See C<mime> above for more details.
...
of a glob pattern. This is available on systems that have I<libpcre2-8>
installed, and supported access control lists. See the C<re> modifier above
...
glob pattern. This is available on systems that have I<libpcre2-8>
...
external shell command to execute via I<system(3)> (i.e., via
