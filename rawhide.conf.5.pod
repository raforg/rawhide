...
Any file specified by I<rh(1)>'s C<-f> option is then read, followed by the
contents of I<rh(1)>'s C<-e> option argument, which can also contain
...
permissions. The "standard" library provides the C<type> and C<perm>
functions (among others) for extracting the file type and permissions from
C<mode>.
...
the modifier name. The following modifiers exist, but not all are available
on all systems.
...
I<chattr(1)>, I<lsattr(1)>, I<jq(1)>, I<The C programming language>.
