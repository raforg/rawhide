...
RAWHIDE_CONFD_LINUX_ATTRIBUTES_RESERVE = attributes.reserve
RAWHIDE_CONFD_LINUX_ATTRIBUTES = attributes
...
# then run gcov *.c then examine *.c.gcov or run gcov_summary (96.55% on Linux)
...
uninstall-config:
	...
	[ "`uname`" != Linux ] || [ ! -f $(DESTDIR)$(APP_CFGDIR)/$(RAWHIDE_CONFD)/$(RAWHIDE_CONFD_LINUX_ATTRIBUTES) ] || rm $(DESTDIR)$(APP_CFGDIR)/$(RAWHIDE_CONFD)/$(RAWHIDE_CONFD_LINUX_ATTRIBUTES)
...
help:
	...
	@echo 
	@echo "To run tests for test coverage analysis (96.55% on Linux):"
	@echo
